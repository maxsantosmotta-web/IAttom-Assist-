...
/* ── TEMPORARY CLERK CLEANUP — AUTHORIZED ONE-TIME USE ─────────────────────
   Deletes test users from Clerk + DB. NEVER touches maxsantosmotta@gmail.com.
   MUST be removed and redeployed immediately after use.                      */
router.post("/admin/cleanup-clerk-users", async (req, res): Promise<void> => {
  ...
  // 1. List all Clerk users
  const clerkList = await clerkClient.users.getUserList({ limit: 100 });
  const allClerkUsers = clerkList.data.map((u) => ({
    id: u.id,
    email: u.emailAddresses.find((e) => e.id === u.primaryEmailAddressId)?.emailAddress
      ?? u.emailAddresses[0]?.emailAddress,
  }));

  // 2. Identify protected user
  const protected_ = allClerkUsers.find((u) => u.email === PROTECTED_EMAIL);
  if (!protected_) {
    res.status(500).json({ error: "Protected user not found in Clerk — aborting" });
    return;
  }

  // 3. Identify test users (everyone except protected)
  const toDelete = allClerkUsers.filter((u) => u.email !== PROTECTED_EMAIL);

  // 4. Delete each test user from Clerk
  const deletedFromClerk: { id: string; email: string | undefined; status: string }[] = [];
  for (const u of toDelete) {
    try {
      await clerkClient.users.deleteUser(u.id);
      deletedFromClerk.push({ id: u.id, email: u.email, status: "deleted" });
    } catch (err: unknown) {
      deletedFromClerk.push({ id: u.id, email: u.email, status: `error: ${err instanceof Error ? err.message : String(err)}` });
    }
  }

  // 5. Clean up any leftover DB records for those Clerk IDs
  const clerkIds = toDelete.map((u) => u.id);
  let dbRemoved = 0;
  if (clerkIds.length > 0) {
    const { inArray: inArr } = await import("drizzle-orm");
    const removed = await db.delete(users).where(inArr(users.clerkId, clerkIds)).returning({ id: users.id });
    dbRemoved = removed.length;
  }

  // 6. Return final state
  const [dbProtected] = await db.select({ id: users.id, email: users.email, role: users.role, plan: users.plan, credits: users.credits }).from(users).where(eq(users.email, PROTECTED_EMAIL));

  // Verify Clerk count after deletion
  const afterClerk = await clerkClient.users.getUserList({ limit: 100 });

  res.json({
    before: { clerkCount: allClerkUsers.length, users: allClerkUsers },
    protected: { ...protected_, db: dbProtected, message: "UNTOUCHED" },
    deletedFromClerk,
    dbRemoved,
    after: {
      clerkCount: afterClerk.data.length,
      clerkUsers: afterClerk.data.map((u) => ({
        id: u.id,
        email: u.emailAddresses.find((e) => e.id === u.primaryEmailAddressId)?.emailAddress,
      })),
    },
